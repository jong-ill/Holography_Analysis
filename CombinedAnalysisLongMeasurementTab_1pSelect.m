...
fname=[path,name];
fname_1 = fname; % Save this for later when we load again
filenum=1;minimum = 0; 
while exist(fname,'file')==2
...
figure();
imgtot=subplot(1,2,1);
imagesc(mean(Stack,3));hold on; colormap('gray');title('Stack average');

imgmean=subplot(1,2,2);
diffim = mean(Diff,3);
imagesc(diffim);hold on; colormap('gray');title(['Average ',num2str(postfigs),' figs post stim - ',num2str(prefigs),' figs pre stim ']);
...
xsave=round(x); ysave=round(y);

%% building ROIs
pattern = ones(512);
roi_size = 20;
sizesave = repmat(roi_size,1,length(xsave));
% Seperating the pattern to the different cells and show figs on subplots
[ Xcoordinates , Ycoordinates ] = Cam_spot_builder( pattern, sizesave, xsave ,ysave  );
spot_num=size(Xcoordinates,2);
[c1,d1]=cellfun(@size ,Xcoordinates);[c2,d2]=cellfun(@size ,Ycoordinates);
