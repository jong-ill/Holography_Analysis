% LeaveOneOutAnalysis.m
%   This script compares the average response of holograms containing N-1 patches to 
%	a full hologram containing N patches. You can either load tiffs directly or import 
%	extracted traces. 
%	JG Edit: For now extracted traces are required 
...
%Omitpre=2;
%% Load the full hologram
 [fnamePat, fpathPat]=uigetfile('','Please choose the pattern file','/gpfs/scratch/jvg219/inhibitory_plasticity/JG2101/200305');
...
%Pull all the holograms and make sets of pixels for averaging. 
...
% Declare varibles for neuropil subtraction
SpotidxBG{spot_num}=[];
SpotMatBG=cell(spot_num,1);
SpotMatBG(:)={zeros(512)};
...
% Create doughnuts for neuropil subtraction. 
for idx1 = 1:spot_num
    % Define doughnut parameters
    ringsize = 30;
...
%     spots.sizesAll(idx1) = sizesave;
    ...
    % Get coordinates for each pattern. Coordinates
    [Xcoordinates_inner, Ycoordinates_inner] = ...
        Cam_spot_builder(pattern, sizesave+10, xsave ,ysave);
    
    % Get coordinates for each pattern. Coordinates
    [Xcoordinates_bg, Ycoordinates_bg] = ...
        Cam_spot_builder(pattern, sizesave+ringsize, xsave ,ysave);
    
%     spots.XcoordinatesAllBG{idx1} = Xcoordinates_bg;
%     spots.YcoordinatesAllBG{idx1} = Ycoordinates_bg;

    SpotidxInner_temp{idx1}=sub2ind(size(pattern),Xcoordinates_inner{:},...
        Ycoordinates_inner{:});
     
    % Create lookup for spots
    SpotidxBG_temp{idx1}=sub2ind(size(pattern),Xcoordinates_bg{:},...
        Ycoordinates_bg{:});
    
    SpotidxBG{idx1} = nonzeros(SpotidxBG_temp{idx1} .* ~ismember(SpotidxBG_temp{idx1},SpotidxInner_temp{idx1}));
    SpotMatBG{idx1}(SpotidxBG{idx1})=1;
...
[fnameH5, fpathH5]=uigetfile('*.h5',...
    'Please choose the hdf5 file of the experiment','C:\VoyeurData');
...
[name,path]=uigetfile('*.tif',...
    'Please choose the first Tiff Stack files of this session','X:\');
...
parfor idx = 1:length(tiffInfo.filelist)
    ...
    for frame = 1:num_images_temp
        ...
        F_bg_temp{idx}(frame,:) = cellfun(@(x) mean(tempFig(x)),SpotidxBG);
    end
...
[ stimTrig{cellNum}.normDiff(1:tiffInfo.width,1:tiffInfo.height,:), stimTrig{cellNum}.Diff(1:tiffInfo.width,1:tiffInfo.height,:),stimTrig{cellNum}.PreStim(1:tiffInfo.width,1:tiffInfo.height,:),...
    stimTrig{cellNum}.PostStim(1:tiffInfo.width,1:tiffInfo.height,:)] = DiffAvgFigsLongMeasurements( m, Stack ,frameidxtemp ,prefigs ,postfigs,Omitpre,Omitpost );
end
...
subtightplot(max(1,ceil(numCells/5)),5,cellPlot,[0.001,0.001])
% subplot(2,max(1,(numCells/2)),cellPlot);
prepost = mean(stimTrig{cellPlot}.Diff,3)./mean(max(abs(mean(stimTrig{cellPlot}.Diff,3))));
    if sigtest == 1
