...
%% Z-score the pixel values
% Comput the std and mean for every pixel in the movie. try turning
    % pixel values into z-scores then taking the mean of the z-scored
    % values. Not sure where this should go. 
...
for i = 1:intervals
    if i==1
        ...
        Omitpost=Omitpost+postfigs;
    end
stimTrig = [];
stimTrig = stimtrigavg(patternTrials,m, tiffInfo, Stack ,frameidx ,prefigs ,postfigs,Omitpost);

for cellPlot = 1:numCells
    cellDiff{cellPlot} = mean(stimTrig{cellPlot}.Diff,3);
    %  dropped the normalization, should do this on the final product. 
%     cellDiff{cellPlot} = mean(stimTrig{cellPlot}.Diff,3)./max(abs(mean(stimTrig{cellPlot}.Diff,3)));

end

% TODO get block number from H5
save(['block1_diffavg_' num2str(i) '.mat'], 'cellDiff', '-v7.3')
end


%% Load sequence and make average
fpathdiff=uigetdir('.','Please choose the folder containing images');

intervals = 10; %find a way to get this from the filenames
for i = 1:intervals
    fnamesdiff{i} = dir(fullfile(fpathdiff,['*' num2str(i) '.mat']));
    numblocks = size(fnamesdiff{1});
    for j = 1:numblocks
        interval{i,j} = load(fullfile(fpathdiff,fnamesdiff{i}(j).name));
    end
end

numCells = size(cellDiff,2);

for i = 1:intervals
for cellNum = 1:numCells
        for j = 1:numblocks
            tempim(:,:,j) = interval{i,j}.cellDiff(cellNum);
        end
        intervalMean{i,cellNum} = mean(cell2mat(tempim),3);
%         intervalStd{i,cellNum} = std(cell2mat(tempim),3); 
end
end



%% Plot sequence average responses 

for cellPlot = 1:numCells
     figure
for intervalplot = 1:size(intervalMean)

subtightplot(ceil(size(intervalMean,1)/5),5,intervalplot,[0.001,0.001])
% subplot(2,max(1,(numCells/2)),cellPlot);
prepost = intervalMean{intervalplot,cellPlot}./max(abs(intervalMean{intervalplot,cellPlot}));
imagesc(medfilt2(prepost,[6 6]));hold on; colormap('gray');
title(['Cell ' num2str(cellPlot) ' ' num2str((intervalplot*100)-100) '-' num2str(intervalplot*100) 'ms']);
...
imagesc(stdImg);hold on; colormap(h6,'gray');title('Stack average');
caxis([0 450]);
axis equal
axis tight
hold on
...
% % subplot(2,max(1,(numCells/2)),cellPlot);
% prepost = mean(stimTrig{cellPlot}.Diff,3)./max(abs(mean(stimTrig{cellPlot}.Diff,3)));
%     if sigtest == 1
